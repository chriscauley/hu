$(function() {
  ...
  $('#Filter').buttonset();
  var image = new Image();
  image.src = './images/lena_gray.jpg';
  image.addEventListener('load', function(e) {
    ...
    $('#Filter').change(function(e) { apply(e.target.value); });
    $('#Slider').slider({ change: function(e, ui) { apply($('input[name=filter]:checked').val()); } });
    ...
    function apply(type) {
      ...
      if(type == 'HPF') {
        ...
        FrequencyFilter.BPF(re, im, 20,radius);
      }
      if(viewtype == "0") {
    ...
    apply = alert_error(apply);

    function checkTypedArray() {
      try { var u8 = new Uint8Array(1), f64 = new Float64Array(1); }
